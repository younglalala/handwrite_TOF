...
class TOrF:
    def __init__(self):
        self.x=tf.placeholder(dtype=tf.float32,shape=[1,64,64,1],name='input')
        self.y_=tf.placeholder(dtype=tf.float32,shape=[1,2])
    ...
    def forward(self):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            net = tf.nn.relu(slim.conv2d(self.x, 10, [3, 3], stride=2))
            net = slim.max_pool2d(net, [2, 2], padding='SAME')
            net = tf.nn.relu(slim.conv2d(net, 16, [3, 3], stride=2))
            net = slim.max_pool2d(net, [2, 2], padding='SAME')
            net = tf.nn.relu(slim.conv2d(net, 32, [3, 3], stride=1))
            net = slim.max_pool2d(net, [2, 2], padding='SAME')
            ...
            # net = slim.max_pool2d(net, [2, 2], padding='SAME')
            print(net)
            net = tf.reshape(net, [-1, 2 * 2 * 32])
            net = tf.nn.relu(slim.fully_connected(net, 32))
...
if __name__=='__main__':
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, './modle_save/m_cls.dpk')
        for ii in range(30000):
            ...
            #     feed_dict={net.x: train_img1, net.y_: train_label,net.dp:0.3})
            # tr_acc=[]
            ...
            if ii%200==0:
                ...
                # open("m_cls.tflite", "wb").write(tflite_model)
