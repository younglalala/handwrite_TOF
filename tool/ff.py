...
def ift1d(RP, IP, dtype="uint8"):
    ...
    for x in range(RP.shape[0]):
        ...
        for u in range(RP.shape[0]):
            temp += (RP[u] * math.cos(2 * math.pi * (u * x / RP.shape[0])) - IP[u] * math.sin(
                2 * math.pi * (u * x / RP.shape[0])))
        temp /= RP.shape[0]
        f[x] = temp
        ...
        for y in range(RP.shape[1]):
            ...
            for u in range(RP.shape[0]):
                for v in range(RP.shape[1]):
                    temp += (RP[u][v] * math.cos(2 * math.pi * (u * x / RP.shape[0] + v * y / RP.shape[1])) - IP[u][
                        v] * math.sin(2 * math.pi * (u * x / RP.shape[0] + v * y / RP.shape[1])))
