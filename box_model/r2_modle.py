...
class Modle:
    def __init__(self):
        ...
        self.y_=tf.placeholder(dtype=tf.float32,shape=[None,7,8])
        ...
        self.fc1_w=tf.Variable(tf.random_normal([1*5*32,64],dtype=tf.float32,stddev=tf.sqrt(1 / 64)))
        self.fc1_b=tf.Variable(tf.zeros([64]))

        self.fc2_w=tf.Variable(tf.random_normal([64,7*8],dtype=tf.float32,stddev=tf.sqrt(1 / 7*8)))
        self.fc2_b=tf.Variable(tf.zeros([7*8]))
    ...
    def forward(self):
        ...
        self.out=tf.reshape(self.out,[-1,7,8])
    ...
    def backward(self):
        ...
        self.argamx_label=tf.argmax(self.y_,axis=2)
        self.argamx_out= tf.reshape(tf.argmax(self.out,axis=2),[-1,7],name='output')
...
if __name__=='__main__':
    ...
    test_data = test_shuffle_batch(test_filename, [32, 168, 1], 20000)
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess,'./save/r6.dpk')
        for i in range(100000):
            train_img,train_label,tarin_file=sess.run(train_data)
            train_img1=train_img/255-0.5
            train_label=one_hot3(train_label,7,8)

            _,train_loss,train_acc,train_out,train_l,out=sess.run([net.opt,net.loss,net.acc,net.argamx_out,net.argamx_label,net.out],
                     feed_dict={net.x:train_img1,net.y_:train_label,net.dp:0.5})

            print(train_out.shape,train_l.shape,out.shape)
            tr_acc = []
            for train_index in range(len(train_out)):
                if operator.eq(str(train_out[train_index]),str(train_l[train_index])):
                    tr_acc.append(1)
                else:
                    tr_acc.append(0)
            ...
            if i%200==0:
                ...
                test_label = one_hot3(test_label1,7,8)
                ...
                for test_index in range(len(test_out)):
                    # print(test_out[test_index],test_l[test_index])
                    if operator.eq(str(test_out[test_index]),str(test_l[test_index])):
                        tes_acc.append(1)
                        # print(test_out[test_index],test_l[test_index])
                    else:
                        tes_acc.append(0)
                        # misc.imsave('/Users/wywy/Desktop/test_e'+'/'+str(test_out[test_index])+'_'+bytes.decode(test_name[test_index]),test_img.reshape([-1,32,168])[test_index])
                ...
                with tf.gfile.GFile("./r6.pb", 'wb') as f:
                    f.write(output_graph_def.SerializeToString())
                #
                #
                #
                saver.save(sess,'./save/r6.dpk')
...
#     net.forward()
#     net.backward()
#     train_data = train_shuffle_batch(train_filename, [16, 168, 1], 128)
#     test_data = test_shuffle_batch(test_filename, [16, 168, 1], 10000)
#
#     init = tf.global_variables_initializer()
#     saver = tf.train.Saver()
#     x = []
...
#         coord = tf.train.Coordinator()  # 创建一个协调器，管理线程
#         threads = tf.train.start_queue_runners(coord=coord, sess=sess)
#         # saver.restore(sess,'./save/cls_1train.dpk')
#         for i in range(100000):
#             train_img,train_label,tarin_file=sess.run(train_data)
#             train_img1=train_img/255-0.5
...
#             tr_acc = []
#             for train_index in range(len(train_out)):
#                 train_arg=np.where(train_out[train_index]<7)[0]
#                 trainl_arg=np.where(train_l[train_index]<7)[0]
#                 if train_arg.tolist()==trainl_arg.tolist():
#                     tr_acc.append(1)
#                 else:
#                     tr_acc.append(0)
#                     # misc.imsave('/Users/wywy/Desktop/train_e'+'/'+str(train_out[train_index])+'_'+bytes.decode(tarin_file[train_index]),train_img.reshape([-1,16,168]) [train_index])
#
#             train_acc = np.mean(np.array(tr_acc))
#
#             print('train iter :{},  train loss:{},  train acc:{}'.format(i,train_loss,train_acc))
...
#                 test_img, test_label1, test_name = sess.run(test_data)
#                 test_img1 = test_img / 255 - 0.5
#                 # test_label=test_label1.reshape([-1,1])
#                 test_label=one_hot3(test_label1,7,8)
#
#                 test_loss ,test_acc,test_out,test_l= sess.run(
#                     [net.loss,net.acc,net.argamx_out,net.argamx_label],
...
#                 tes_acc = []
#                 for test_index in range(len(test_out)):
#                     test_arg = np.where(test_out[test_index] < 7)[0]
#                     testl_arg = np.where(test_l[test_index] < 7)[0]
#                     if test_arg.tolist() == testl_arg.tolist():
#                         tes_acc.append(1)
#                     else:
#                         tes_acc.append(0)
#                         # misc.imsave('/Users/wywy/Desktop/test_e'+'/'+str(test_out[test_index])+'_'+bytes.decode(test_name[test_index]),test_img.reshape([-1,16,168])[test_index])
#
#                 test_acc = np.mean(np.array(tes_acc))
#
#                 graph_def = tf.get_default_graph().as_graph_def()
...
#                                                                                 ['output'])
#
#                 with tf.gfile.GFile("./cls_1train.pb", 'wb') as f:
#                     f.write(output_graph_def.SerializeToString())
#
#
#
#                 # saver.save(sess,'./save/cls_4train.dpk')
#
#                 print('------------test iter :{},  test loss:{},  test acc:{}----------'.format(i,test_loss,test_acc))
