...
from mobile_model.train_sample import  *
from mobile_model.test_sample import *

from box_model.one_hott import one_hot,one_hot2,one_hot3

class Modle:
    def __init__(self):
        self.x=tf.placeholder(dtype=tf.float32,shape=[None,32,42,1],name='input')
        ...
        self.conv2_w=tf.Variable(tf.random_normal([3,3,10,32],dtype=tf.float32,stddev=tf.sqrt(1 / 32)))
        ...
        self.fc1_w=tf.Variable(tf.random_normal([2*2*32,64],dtype=tf.float32,stddev=tf.sqrt(1 / 64)))
        self.fc1_b=tf.Variable(tf.zeros([64]))

        self.fc2_w=tf.Variable(tf.random_normal([64,2],dtype=tf.float32,stddev=tf.sqrt(1 / 2)))
        self.fc2_b=tf.Variable(tf.zeros([2]))
    ...
    def forward(self):
        ...
        #     tf.nn.conv2d(self.pool2, self.conv3_w, strides=[1, 2, 3, 1], padding='SAME') + self.conv3_b
        # ))
        ...
        print(self.pool2)
        self.flat=tf.reshape(self.pool2,shape=[-1,2*2*32])
        ...
        self.out=tf.reshape(self.out,[-1,2])
...
if __name__=='__main__':
    ...
    net.forward()
    net.backward()
    train_data = train_shuffle_batch(train_filename, [32, 42, 1], 128)
    test_data = test_shuffle_batch(test_filename, [32, 42, 1], 20000)
    ...
    with tf.Session() as sess:
        ...
        for i in range(100000):
            ...
            # train_label=one_hot(train_label.tolist(),2)
            #
            ...
            #     else:
            #         tr_acc.append(0)
            #         # misc.imsave('/Users/wywy/Desktop/train_e'+'/'+str(train_out[train_index])+'_'+bytes.decode(tarin_file[train_index]),train_img.reshape([-1,32,42]) [train_index])
            #
            ...
            if i%200==0:
                ...
                test_label = one_hot(test_label1.tolist(),2)
